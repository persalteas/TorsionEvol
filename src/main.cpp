...
int main(int argc, char** argv) {
	...
	prot_file* prot = readProt(pth + params->BARR_FIX);
	TTS_file* tts = readTTS(pth + params->TTS); // TTS means "Transcription Termination Site"
	TSS_file* tss = readTSS(pth + params->TSS); // TSS means "Transcription Start Site"
	GFF_file* gff_df_raw = readGFF(pth + params->GFF);
	...
	vector<int> relative_size (tr_id.size());
	std::transform(tr_end.begin(), tr_end.end(), tr_start.begin(), relative_size.begin(), minus<int>());
	std::transform(relative_size.begin(), relative_size.end(), tr_size.begin(), ::abs);
