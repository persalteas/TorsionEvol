...
double Individual::_mean = 5, Individual::_p = 0.5;   // modifiable with command line arguments 3 and 4
...
Individual::Individual(Individual&& indiv)
{
  ...
  _barr_fix = indiv._barr_fix; // undefined behaviour when indiv will be deleted ?
  _fitness = indiv._fitness;
}
...
void Individual::set_simulation_params(Params* params)
{
  _params = params;
}

void Individual::set_target_envir(vector<double>& env)
{
    _target_envir = env;
}
...
void Individual::estimate_exression()
{
    ...
    std::vector<double> init_rates, prob_init_rates, prob_unhooked_rates, all_prob;
    ...
    for (size_t t=0; t<(size_t)_params->ITERATIONS_NB; ++t)
    {
        ...
        for (auto it = _genes->begin(); it!=_genes->end(); it++)
            init_rates.push_back(it->f_init_rate(_params->sigma_t, _params->epsilon, _params->m, Barr_pos, Barr_sigma));
        // compute the probabilities of expression of each tr
        sum_init_rates = vector_sum(init_rates);
        for (auto it = _genes->begin(); it!=_genes->end(); it++) 
            prob_init_rates.push_back(it->f_prob_init_rate(sum_init_rates, _params->DELTA_T));
        ...
        if (N_RNAPs_unhooked) 
        {
            ...
            prob_unhooked_rate = f_prob_unhooked_rate(sum_init_rates, _params->DELTA_T, N_RNAPs_unhooked);
            ...
            iota(tss_and_unhooked_RNAPs.begin(), tss_and_unhooked_RNAPs.begin() + _genes->size(), 0);
            // concatenation of the probabilities arrays
            copy(prob_init_rates.begin(), prob_init_rates.end(), all_prob.begin());
            copy(prob_unhooked_rates.begin(), prob_unhooked_rates.end(), all_prob.begin()+_genes->size());
            ...
            for (int i : picked_tr)
            {
                ...
                if (i != -1)
                {
                    ...
                    pos = (*_genes)[i].start_;
                    index = std::find_if(Barr_pos.begin(), Barr_pos.end(), 
        ...
        for (uint i = 0; i<Barr_pos.size(); i++)
        {
            ...
            switch (a)
            {
                case 0:
                    switch (b) 
                    {
                        case 1: Barr_sigma[i] = Barr_sigma[i] * (d-1.0)/d - _RNAPs_genSC/d; break;
                        case -1: Barr_sigma[i] = Barr_sigma[i] * (d+1.0)/d + _RNAPs_genSC/d; 
                    }
                    break;
                case -1:
                    switch (b) 
                    {
                        case 1: Barr_sigma[i] = Barr_sigma[i] * (d-2.0)/d - 2*_RNAPs_genSC/d; break;
                        case 0: Barr_sigma[i] = Barr_sigma[i] * (d-1.0)/d - _RNAPs_genSC/d; 
                    }
                    break;
                case 1:
                    switch (b) 
